...
def draw_boxes(img, bbox, names, object_id, identities=None, offset=(0, 0)):
    ...
    # cv2.line(img, (0, line_y), (width, line_y), (0, 255, 0), 2)  # 绿色线
    ...
    for i, box in enumerate(bbox):
        ...
        if y1 < line_y or y2 < line_y:
            x1 += offset[0]
            x2 += offset[0]
            y1 += offset[1]
            y2 += offset[1]

            object_height = y2 - y1  # 对象在图像中的像素高度
            obj_name = names[object_id[i]]

            # 调用相应的距离计算函数
            if obj_name == "bicycle":
                distance = detect_distance_bicycle(object_height)
            elif obj_name == "car":
                distance = detect_distance_car(object_height)
            elif obj_name == "motorcycle":
                distance = detect_distance_motorcycle(object_height)
            elif obj_name == "bus":
                distance = detect_distance_bus(object_height)
            elif obj_name == "truck":
                distance = detect_distance_truck(object_height)
            else:
                distance = 0  # 对于未知对象，可以设置为0或其他默认值

            # 将距离信息添加到标签中
            id = int(identities[i]) if identities is not None else 0
            color = compute_color_for_labels(object_id[i])
            label = '{}{:d}: {} {:.2f}m'.format("", id, obj_name, distance)
